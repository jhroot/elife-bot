...
class workflow(object):
        ...
        for step in self.definition["steps"]:
            ...
            if type(step) == list:
                ...
                for p_activity in step:
                    ...
                    if not self.activity_status(self.decision, activityType, activityID):
                        return False
            else:
                # A single activity
        ...
        for step in self.definition["steps"]:
            ...
            if type(step) == list:
                ...
                for p_activity in step:
                    ...
                    if not self.activity_status(self.decision, activityType, activityID):
                        all_completed = False
                    if self.activity_status(self.decision, activityType, activityID):
                        none_started = False
                if all_completed is False and none_started is True:
    ...
    def activity_status(self, decision, activityType=None, activityID=None):
        ...
        for event in decision["events"]:
            ...
            if activityType is not None and activityID is not None:
                try:
                    if (event["activityTaskScheduledEventAttributes"]["activityType"]["name"] == activityType and
                            event["activityTaskScheduledEventAttributes"]["activityId"] == activityID):
    ...
    def get_input(self):
        ...
        if self.decision is None:
            return None
        try:
            input_data = json.loads(self.decision["events"][0]["workflowExecutionStartedEventAttributes"]["input"])
        except KeyError:
            input_data = None
        return input_data
